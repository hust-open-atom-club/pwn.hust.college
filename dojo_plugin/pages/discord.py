...
from ..utils.discord import (
    OAUTH_AUTHORIZE_ENDPOINT, get_discord_id, get_discord_member, add_role,
    DiscordChannels, send_channel_message, send_user_message
)
...
def discord_connect():
    ...
    params = dict(client_id=DISCORD_CLIENT_ID,
                  ...
                  scope="identify",
                  state=state)
    oauth_url = requests.Request("GET", OAUTH_AUTHORIZE_ENDPOINT, params=params).prepare().url
...
def discord_redirect():
    ...
    send_channel_message(
        f"<@{discord_id}> 欢迎加入 pwn.hust.college 的大家庭！",
        DiscordChannels.WELCOME
    )
    send_user_message(
        f"你的 Discord 账号已经和用户 {user.name} 绑定。",
        discord_id
    )
...
@discord.route("/discord/disconnect")
@authed_only
def discord_disconnect():
    ...
    if discord_user:
        ...
        send_user_message(
            "当前 Discord 账号已经和 pwn.hust.college 解绑，如果不是你的操作，请关注账号安全。",
            discord_id
        )
